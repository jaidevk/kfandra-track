...
export default function HomeScreen({ isStaff }: { isStaff: boolean }) {
  const { brand, home, admin } = strings;
  return (
      ...
      <main className="relative z-10 flex w-full max-w-md flex-col items-center gap-10 text-center">
        ...
        {isStaff && (
          ...
          >
            <Link
              href="/admin"
              className="inline-flex items-center gap-1.5 rounded-full border border-white/30 bg-white/10 px-4 py-1.5 text-[11px] font-semibold uppercase tracking-wide text-white backdrop-blur hover:bg-white/20"
            >
              {admin.title} →
            </Link>
          </motion.div>
